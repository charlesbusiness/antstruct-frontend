...
import { Container, Typography, Grid, CardContent, Box, Button, TextField, Dialog, DialogTitle, DialogContent, Divider } from "@mui/material";
...
export default function Departments() {
  ...
  const openCreateDept = () => {
    if (!businessUserProfile) {
      toast.error('Please create a business profile to continue', { autoClose: 3000 })
      setTimeout(() => {
    ...
    <>
      <Box sx={{ mt: 2 }}>
        <Grid sx={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <Button variant="contained" onClick={openCreateDept}>Add New</Button>
        </Grid>
        <Divider sx={{ my: 1 }} />
        <Grid container spacing={2}>
          {departments?.map((department) => (
            <Grid item xs={12} sm={6} md={4} key={department.id} sx={{ display: 'flex', justifyContent: 'space-between', }}>
